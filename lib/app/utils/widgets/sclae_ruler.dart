...
class SimpleRulerPicker extends StatefulWidget {
  ...
  final double height;
  final double pointerFontSize;
  final double pointerIconSize;
  ...
  const SimpleRulerPicker({
    ...
    required this.pointerFontSize,
    required this.pointerIconSize,
  }) : assert(
...
class SimpleRulerPickerState extends State<SimpleRulerPicker> {
  ...
  bool onNotification(ScrollNotification scrollNotification) {
    if (scrollNotification is ScrollEndNotification) {
      ...
      if (!_isPosFixed) {
        final jumpIndex = getScrolledItemIndex(
          scrollPixels,
          widget.scaleItemWidth,
        );
        ...
        Future.delayed(const Duration(milliseconds: 100)).then((_) {
          _isPosFixed = true;
          _scrollController.jumpTo(jumpPixels);
        });
      }
    }
    return true;
      ...
      children: [
                    ...
                    padding:
                        _isHorizontalAxis
                            ? EdgeInsets.only(
                              ...
                              right: (constraints.maxWidth / 2)-6,
                            )
                            : EdgeInsets.only(
                              top: (constraints.maxHeight / 2),
                              bottom: (constraints.maxHeight / 2)-6,
                            ),
        ...
        _isHorizontalAxis
            ? _VerticalPointer(
              ...
              scaleLabelSize: widget.scaleLabelSize,
              scaleBottomPadding: widget.scaleBottomPadding,
              fontSize: widget.pointerFontSize,
              iconSize: widget.pointerIconSize,
              lineStroke: widget.lineStroke,
            )
            : _HorizontalPointer(
              selectedValue: _selectedValue,
    ...
    return Align(
      ...
      child: Row(
        ...
        children: [
          ...
          // ),
          // Icon(Icons.arrow_right, color: selectedColor, size: iconSize),
          Container(
            height: lineStroke,
    ...
    return Align(
      ...
      child: Column(
        ...
        children: [
          ...
          //   style: TextStyle(fontSize: fontSize, color: selectedColor),
          // ),
          // Icon(Icons.arrow_drop_down, color: selectedColor, size: iconSize),
          Container(
            height: longLineHeight,
            width: lineStroke,
            color: selectedColor,
          ),
          SizedBox(height: scaleLabelSize + scaleBottomPadding),
        ],
      ),
    );
  }
